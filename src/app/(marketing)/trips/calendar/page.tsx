...
import { LayoutGrid, CalendarDays, Search, Filter, ChevronLeft, ChevronRight, Info } from "lucide-react";
...
export default async function CalendarPage({
  ...
  // Default to October 2024 since that's where most seed data is
  const targetMonth = resolvedParams.month ? new Date(resolvedParams.month) : new Date("2024-10-01");
  const monthName = targetMonth.toLocaleString('default', { month: 'long', year: 'numeric' });
  
  // Get all departures for this month
  ...
  const departures = await prisma.departure.findMany({
    where: {
      ...
      trip: category !== "All" ? { category } : undefined,
    },
    include: {
      trip: true,
    },
  ...
  const departuresByDate = departures.reduce((acc, dep) => {
    const dateStr = dep.date.toISOString().split('T')[0];
  ...
  // Pick a date to show in the sidebar (default to first date with departures or just day 15)
  const dateWithDepartures = Object.keys(departuresByDate)[0];
  const selectedDateStr = dateWithDepartures || `${targetMonth.getFullYear()}-10-15`;
  ...
  return (
    <div className="bg-slate-50 min-h-screen pb-24">
      {/* Header */}
      <div className="bg-navy-900 pt-12 pb-24 px-6 relative overflow-hidden">
        <div className="absolute inset-0 opacity-20 bg-[url('https://source.unsplash.com/random/1920x1080/?mountain,forest')] bg-cover bg-center mix-blend-overlay" />
        ...
        <div className="bg-white rounded-2xl shadow-lg p-4 md:p-6 flex flex-col md:flex-row items-center justify-between gap-4 border border-slate-100">
          ...
          <div className="flex items-center gap-2 overflow-x-auto w-full md:w-auto pb-2 md:pb-0 hide-scrollbar">
            {categories.map((cat) => (
              <Link
                key={cat}
                href={`/trips/calendar${cat !== "All" ? `?category=${cat}` : ""}`}
                    ...
                    >
                      ...
                      {hasDepartures && (
                        <div className="flex flex-col gap-1.5">
                          {departuresByDate[dateStr].slice(0, 3).map((dep, i) => {
                            let dotColor = "bg-success";
                            if (dep.status === "Filling Fast") dotColor = "bg-warning";
                            if (dep.status === "Almost Full") dotColor = "bg-danger";
                            
                            return (
                              <div key={i} className="flex items-center gap-1.5 px-2 py-1 rounded bg-slate-50 border border-slate-100 overflow-hidden">
                  ...
                  selectedDepartures.map(dep => (
                    <div key={dep.id} className="card-elevated bg-white rounded-2xl overflow-hidden border border-slate-200">
                      <div className="flex items-stretch h-28">
                        ...
                        <img 
                          src={dep.trip.imageUrl || `https://source.unsplash.com/random/400x300/?${dep.trip.category.toLowerCase()}`} 
                          alt={dep.trip.name}
                        ...
                        <div className="w-2/3 p-4 flex flex-col justify-center">
                          ...
                          <div className="flex items-center justify-between mt-auto">
                            <span className="font-bold text-navy-900">${dep.trip.price}</span>
                      ...
                      <div className="p-3 border-t border-slate-100 bg-slate-50">
                        <Link href={`/trips/${dep.trip.slug}`} className="btn-primary w-full py-2.5 text-sm">
                          View Trip Details
                        </Link>
                      </div>
                    </div>
