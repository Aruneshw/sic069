...
export default async function TripsPage({
  ...
  // Build query
  const whereClause = category !== "All" ? { category, status: "Published" } : { status: "Published" };
  ...
  return (
    <div className="bg-slate-50 min-h-screen pb-24">
      {/* Hero — Full-width travel map image */}
      <div className="relative overflow-hidden" style={{ minHeight: "420px" }}>
        {/* Background image */}
        <img
          src="/images/trips-hero.png"
          alt="Travel planning flat lay with world map, compass, globe, and postcards"
        ...
        <div className="relative z-10 flex flex-col items-center justify-center text-center px-6 py-28 md:py-36">
          <span className="inline-block px-4 py-1.5 rounded-full bg-white/15 backdrop-blur-md border border-white/20 text-xs font-semibold text-teal-200 uppercase tracking-widest mb-5">
            ✦ Curated Regional Tours
          </span>
          <h1 className="text-4xl md:text-6xl lg:text-7xl font-black text-white mb-5 drop-shadow-lg leading-[1.1]">
          ...
          <p className="text-lg md:text-xl text-white/80 max-w-2xl mx-auto drop-shadow-sm font-medium">
            Browse our complete collection of regional tours, from relaxing coastal retreats to challenging alpine summits.
          </p>
        </div>
      </div>

      {/* Controls Bar - Negative margin to overlap header */}
      <div className="container-main relative -mt-8 z-20 mb-12">
        <div className="bg-white rounded-2xl shadow-lg p-4 md:p-6 flex flex-col md:flex-row items-center justify-between gap-4 border border-slate-100">
          ...
          <div className="flex items-center gap-2 overflow-x-auto w-full md:w-auto pb-2 md:pb-0 hide-scrollbar">
            {categories.map((cat) => (
              <Link
                key={cat}
                href={`/trips${cat !== "All" ? `?category=${cat}` : ""}`}
