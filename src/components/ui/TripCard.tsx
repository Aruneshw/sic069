...
export default function TripCard({ trip, index = 0 }: TripCardProps) {
  ...
  return (
    <motion.div
      ...
      className="card-3d relative w-full h-full group"
    ...
    >
      <Link href={`/trips/${trip.slug}`} className="block no-underline h-full">
        <motion.div
          style={{ transform }}
          className="card-3d-inner card-elevated flex flex-col h-full overflow-hidden bg-white"
        >
          {/* Image Container */}
          <div className="relative aspect-[4/3] w-full overflow-hidden">
            ...
            <img
              src={trip.imageUrl || `https://source.unsplash.com/random/800x600/?${trip.category.toLowerCase()}`}
              alt={trip.name}
